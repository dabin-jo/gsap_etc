...
// Lenis는 마우스휠 스크롤을 부드럽게 만들어준다
// GSAP ScrollTrigger와 같이 쓰면 스크롤 애니메이션도 더 자연스럽게 붙는다
const lenis = new Lenis({
    // 자동 실행 대신 우리가 직접 프레임을 돌리게 한다
    autoRaf: false,
...
// 매 프레임마다 lenis와 scrollTrigger를 같이 업데이틀 해야 스크롤 애니메이션이 어긋나지 않음
function raf(time) {
    // 지금 프레임의 시간을 lenis에게 알려준다
    lenis.raf(time);
    // ScrollTrigger가 현재 스크롤 위치를 다시 계산하게 한다
    ScrollTrigger.update();
    // 다음 화면 프레임에서도 같은 일을 반복한다
...
// 메뉴 링크를 누른 뒤에는 모바일 메뉴를 닫아 화면을 다시 넓게 보여준다
...
// 맨 아래 근처에서는 contact 메뉴가 끝까지 active로 보이게 따로 처리한다
function updateActiveFromViewport() {
    ...
    // 아직 못 찾았을 때 기본 active는 hero로 둔다
    let activeHref = '#hero';
    ...
    sectionMap.forEach((id) => {
        ...
        // 기준점이 섹션 안에 들어와 있는지 확인한다
        const isInside = rect.top <= checkPoint && rect.bottom >= checkPoint;
        // 섹션 안이면 거리 0, 밖이면 위/아래 경계중 더 가까운 거리로 계산한다
...
// 페이지가 처음 열렸을 때는 hero메뉴를 active로 시작한다
...
function marquee() {
    ...
    document.querySelectorAll('.marquee_wrap').forEach((wrap) => {
        ...
        // 기준 줄이 없으면 복사할 것도 없으니 건너 뛴다
        if (!row) return;

        // 한 줄의 실제 가로 길이를 기억한다
        const rowWidth = row.scrollWidth;

        // 같은 줄을 여러개 복제해서 빈틈없이 이어붙인다
        ...
        function step() {
            // reverse 클래스가 있으면 오른쪽, 없으면 왼쪽으로 움직이게 한다
...
gsap.to('.feature_card', {
    ...
    duration: 0.6,
    ease: "power2.out",
    stagger: 0.22, // 시간차로 올라오게 하는 것
    scrollTrigger: {
        trigger: '.feature_list',
        start: 'top 55%',
        // onEnter, onLeave, onEnterBack, onLeaveBack
        // 내려갈 때 재생, 위로 벗어나면 다시 되감기
...
function splitWords(selector, wordClass) {
    ...
    document.querySelectorAll(selector).forEach((textEl) => {
        // textEl 요소 안의 텍스트를 가져와 앞뒤 공백을 제거한 뒤에 공백 기준으로 나누어 배열로 만드는 코드
...
// 설명문장도 motion_word 단어 span으로 나눈다
...
document.querySelectorAll('.feature_card').forEach((card) => {
    ...
    card.addEventListener('mousemove', (e) => {
        ...
        glass.style.background = `radial-gradient(circle at ${x}px ${y}px,rgba(240,222,68, 0.2),rgba(196,196,68, 0) 30%)`;
    });
...
document.querySelectorAll('.tit_card').forEach((card) => {
    ...
    // 카드 위에서 마우스를 움직일 때 기울기 값을 계산한다
    card.addEventListener('mousemove', (e) => {
        const rect = card.getBoundingClientRect();
        // 카드 안에서 마우스가 가로와 세로로 몇퍼센트 위치인지 계산
        ...
        // 계산한 각도를 안쪽 레이어 transform에 적용한다
...
// 사진마다 살짝 다른 각도와 giltch 효과를 줘서 디자인 갤러리처럼 보이게 한다
photos.forEach((photo, index) => {
    // 사진이 등장하기 직전에 레이어 순서와 gilch 클래스를 조정
    pinTl.add(() => {
        // 뒤에 나온 사진이 앞쪽에 쌓이도록 z-index를 올린다
        photo.style.zIndex = String(100 + index);
        photo.classList.add('glitch');
        // 0.4초 뒤에 glitch 클래스를 제거해서 효과를 끝낸다
...
// track 전체 너비에서 화면에 이미 보이는 너비를 빼면 이동 거리만 남는다
function totalWidth() {
...
// horizontal_section은 세로 스크롤을 가로 이동으로 바꿔서 보여준다
const horizontalTween = gsap.to('.track', {
    ...
    scrollTrigger: {
        ...
        start: 'top top',
        // 실제 이동 거리보다 더 길게 잡아서 앞뒤 여백 스크롤을 만든다
...
document.querySelectorAll('.case_panel').forEach((article, articleIndex) => {
    ...
    // 패널 안의 이미지들을 하나씩 다른 preset으로 등장
...
// 마지막 cta버튼도 화면에 들어오면 자연스럽게 나타난다
...
window.addEventListener('load', () => {
    ...
    if(document.fonts){
        // 폰트가 모두 준비된 뒤에 scrolltrigger위치를 다시 맞춘다
